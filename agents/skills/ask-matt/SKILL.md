...
3. **`/to-blueprint`** — the interview settled the *decisions*; this settles the **drawing**. Structure, names and logic, at signature depth, confirmed one pass at a time before any code exists. It's where a mismatch between your head and the agent's gets caught for the price of a markdown line, and where the file paths and signatures live that `/to-spec` and `/to-tickets` deliberately keep out. Skip it only when the work is small enough that the names are obvious.
...
Keep steps 1–4 in **one unbroken context window** — don't compact or clear until after `/to-tickets` — so the grilling, blueprint, spec, and tickets all build on the same thinking. Each `/implement` then starts fresh, working from the ticket and the blueprint it points at.
